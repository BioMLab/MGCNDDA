...
import numpy as np
import csv
from cla_metric import cv_model_evaluate
import torch as t
...
def train(model, train_data, data_test, association_matrix, optimizer, opt):
    ...
    for epoch in range(1, opt.epoch + 1):
        ...
        np.savetxt('score.csv', score.detach().cpu().numpy(), fmt='%.6f', delimiter=',')
        if epoch % 100 == 0:
...
def main():
    ...
    diseaseSim = read_csv('predict_wdd_GS.csv')
    drugSim = read_csv('predict_wrr_GS.csv')
    dataset['drug_dis'] = read_csv('data_rw_alpha_06_test1.csv')
    ...
    for i in range(opt.validation):
        ...
        for k in range(k_folds-9):
            ...
            np.savetxt('score_matrix.csv', score.detach().cpu().numpy(), fmt='%.6f', delimiter=',')
