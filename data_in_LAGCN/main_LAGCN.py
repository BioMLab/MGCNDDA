...
import numpy as np
import csv
from evaluate_LAGCN import cv_model_evaluate
import torch as t
...
def train(model, train_data, data_test, association_matrix, optimizer, opt):
    ...
    for epoch in range(1, opt.epoch + 1):
        ...
        np.savetxt('score_LAGCN.csv', score.detach().cpu().numpy(), fmt='%.6f', delimiter=',')
        if epoch % 100 == 0:
...
def main():
    ...
    diseaseSim = read_csv('LAGCN_wrr_GS.csv')
    drugSim = read_csv('LAGCN_wdd_GS.csv')
    dataset['drug_dis'] = read_csv('dr_LAGCN.csv')
    ...
    for i in range(opt.validation):
        ...
        index_matrix = np.mat(np.where(value > 1e-5))
        ...
        index_matrix_negative = np.mat(np.where(value <= 1e-5))
